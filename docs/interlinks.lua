...
local function unwrap_backticks(s)
...
local function build_search_object(str, debug)
    ...
    if starts_with_colon then
        ...
        if #t == 2 then
            ...
            search.name = unwrap_backticks(t[2])
        elseif #t == 3 then
            -- e.g. :py:func:`my_func`
            search.domain = t[1]
            search.role = normalize_role(t[2])
            search.name = unwrap_backticks(t[3])
        elseif #t == 4 then
            ...
            search.name = unwrap_backticks(t[4])
        else
            _debug_log("couldn't parse this link: " .. str, debug)
            return {}
        end
    else
        search.name = unwrap_backticks(str)
    end
...
function Link(link)
    ...
    if not (link.target:match("%%60") or link.target:match("`")) then
        return link
    end
    ...
    -- Use %$ to match literal $ in gsub pattern
...
function Code(code)
    ...
    -- return code.attr
    local search = build_search_object("`" .. text .. "`")
    local item = lookup(search)
