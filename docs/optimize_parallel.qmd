...
description: "Step-by-step walkthrough of _execute_optimization_run() and every method it calls along the parallel (steady-state) path, with executable examples validated by pytest."
---

This document traces every step executed by `SpotOptim._execute_optimization_run()` along
...
each cycle to `_execute_optimization_run()`.
When `n_jobs > 1`, that dispatcher routes to `_optimize_steady_state()`,
...
## Step 1 — Dispatch (`_execute_optimization_run()`)
...
`_execute_optimization_run()` is the routing layer between the outer restart loop in
...
`_optimize_steady_state()` (parallel) or `_optimize_sequential_run()` (sequential).
...
## Step 2 — Parallel Run Orchestration (`_optimize_steady_state()`)
...
`_optimize_steady_state()` is the parallel orchestrator.
...
Because all `n_initial` points are dispatched concurrently to worker processes or threads
in the next phase, curation must be complete before submission: worker processes operate
on a snapshot of the optimiser serialised with `dill` and cannot interact with the
main-process state.
...
All `n_initial` design points are submitted to `eval_pool` simultaneously.
On GIL builds, each point is serialised together with a snapshot of the optimiser using
`dill.dumps()`.
The TensorBoard writer is temporarily set to `None` before serialisation because
`SummaryWriter` objects are not picklable; it is restored immediately afterwards.
...
On free-threaded builds the nested function `_thread_eval_task_single()` calls
`evaluate_function()` directly without any serialisation overhead.
...
## Step 8 — First Surrogate Fit (`_fit_scheduler()`)
...
this is the only point in the parallel path where `_fit_scheduler()` is called without
holding `_surrogate_lock`.
`_fit_scheduler()` selects the most recent `window_size` training points according to
...
## Step 9 — Steady-State Loop Overview (`_optimize_steady_state()`)
...
## Step 15 — Batch Evaluation Processing (`update_success_rate()`, `_update_storage_steady()`, `_fit_scheduler()`)
...
After all points in the batch have been stored, `_fit_scheduler()` is called once under
...
`_optimize_steady_state()` exits the while loop when `len(y_) >= effective_max_iter` or
...
|-----:|----------------|---------|
| 1 | `_execute_optimization_run()` | Dispatch to parallel path when `n_jobs > 1` |
| 2 | `_optimize_steady_state()` | Parallel orchestrator; manages pools and phases |
| 3 | `set_seed()`, `get_initial_design()`, `curate_initial_design()` | Seed RNG, generate and curate initial design |
| 4 | `_is_gil_disabled()` | Detect GIL state; select `ProcessPoolExecutor` or `ThreadPoolExecutor` for eval |
| 5 | Phase 1 submission | Submit all `n_initial` points to `eval_pool` concurrently |
...
| 8 | `_fit_scheduler()` | First surrogate fit (no lock needed, no search threads active) |
| 9 | `_optimize_steady_state()` while loop | Main loop: iterate until budget or time exhausted |
...
| 15 | `update_success_rate()`, `_update_storage_steady()`, `_fit_scheduler()` | Process batch result; update storage and best; refit surrogate under lock |
