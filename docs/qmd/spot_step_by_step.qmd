...
print(f"  - rosenbrock: Standard 2D Rosenbrock (imported from spotoptim.function)")
print(f"  - rosenbrock_noisy: With Gaussian noise (σ=0.1)")
...
- `surrogate`: Gaussian Process with Matérn kernel (default)
...
## Phase 7: Storage and Statistics Initialization
...
1. `init_stats()`: Initialize statistics tracking variables. It calls `update_stats()`, which computes mean/variance if noise handling is active.
3. `mean_X`, `mean_y`, `var_y`: Track statistics for noisy functions


* Initialize storage and statistics (as done in `optimize()`)

```{python}
# Initialize storage and statistics using the new init_stats() method
opt.init_stats(X0_clean, y0_clean)
...
* Generate a contour plot with contour lines of the Rosenbrock function and mark the best point. Show all evaluated points as well:
...
- Provides: Mean prediction μ(x) and uncertainty σ(x)
...
print(f"  - Exploitation: Low predicted mean μ(x)")
print(f"  - Exploration: High uncertainty σ(x)")
```
...
if y_next[0] < opt.best_y_:
    ...
    print(f"\n🎉 Improvement found! Δf = {improvement:.6f}")
else:
