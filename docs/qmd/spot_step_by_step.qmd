...
```{python}
if opt.ocba_delta > 0:
    selected_indices = opt._apply_ocba()
    print(f"OCBA selected {len(selected_indices)} points for re-evaluation")
else:
    print("OCBA not applied (ocba_delta=0)")
```
...
Update the number of repeats for infill points based on new evaluations.

**What happens in `_update_repeats_infill_points()`:**

1. Identify infill points (points with high uncertainty)
2. Update their repeat counts based on new evaluations
3. Return updated repeat counts

```{python}
repeats_updated = opt._update_repeats_infill_points()
print(f"Updated repeats for infill points: {repeats_updated}")
```
...
```{python}
opt.update_stats()
print(f"  mean_X shape: {opt.mean_X.shape} (unique points)")
print(f"  mean_y shape: {opt.mean_y.shape} (mean values)")
print(f"  var_y shape: {opt.var_y.shape} (variances)")
```
