...
def test_mo_eval_models_default_score(data):
    ...
    scores, models, preds = mo_eval_models(
        X_train, y_train_df, X_test, y_test_df, make_model
    )
...
def test_mo_eval_models_single_callable(data):
    ...
    scores, models, preds = mo_eval_models(
        X_train, y_train, X_test, y_test, make_model, scores=mean_squared_error
    )
...
def test_mo_eval_models_dict_scores(data):
    ...
    my_scores = {"R2": r2_score, "MSE": mean_squared_error}

    scores_dict, models, preds = mo_eval_models(
        X_train, y_train, X_test, y_test, make_model, scores=my_scores
    )
...
def test_mo_cv_models_single_score_str(data):
    ...
    cv_scores = mo_cv_models(
        X_train, y_train, make_model, cv=3, scores="neg_mean_squared_error"
    )
