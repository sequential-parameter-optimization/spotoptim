...
def test_optimization_message_format():
    ...
    bounds = [(-5, 5), (-5, 5)]
    optimizer = SpotOptim(
        fun=objective, bounds=bounds, max_iter=10, n_initial=5, seed=42
    )
    ...
    assert (
        "Optimization terminated" in res.message
        or "Optimization finished successfully" in res.message
    )
...
def test_optimization_message_max_iter():
    ...
    optimizer = SpotOptim(
        fun=objective, bounds=bounds, max_iter=5, n_initial=5, seed=42
    )
