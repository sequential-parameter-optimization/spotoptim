...
def sample_df_with_nans():
    ...
    return pd.DataFrame({"A": [1, 2, np.nan, 4, 5], "B": [10, np.nan, 30, 40, 50]})
...
def additional_points_df():
    ...
    return pd.DataFrame({"A": [2.5, 7.5], "B": [25, 75]})
...
class TestPlotIpHistogramsAdditionalPoints:
    ...
    def test_with_additional_points(self, mock_show, sample_df, additional_points_df):
        ...
        plot_ip_histograms(
            sample_df, add_points=additional_points_df, add_points_col=["red", "blue"]
        )
        mock_show.assert_called_once()
        plt.close("all")
    ...
    def test_with_partial_additional_points(self, mock_show, sample_df):
        ...
        plot_ip_histograms(
            sample_df, add_points=partial_points, add_points_col=["red", "blue"]
        )
        mock_show.assert_called_once()
        plt.close("all")
    ...
    def test_with_additional_points_with_nans(self, mock_show, sample_df):
        ...
        plot_ip_histograms(
            sample_df, add_points=points_with_nan, add_points_col=["red", "blue"]
        )
        mock_show.assert_called_once()
        plt.close("all")
...
class TestPlotIpHistogramsIntegration:
    ...
    @patch("matplotlib.pyplot.show")
    def test_full_parameter_combination(
        self, mock_show, sample_df, additional_points_df
    ):
...
class TestPlotIpBoxplotsIntegration:
    ...
    @patch("matplotlib.pyplot.show")
    def test_full_parameter_combination(
        self, mock_show, sample_df_with_categories, additional_points_df
    ):
