...
def test_generate_mesh_grid_with_factors_one_factor():
    ...
    assert X_i.shape == (50, 3), "X_i should be 50x3 (50 lr points × 3 activation levels)"
    assert X_j.shape == X_i.shape, "X_j should have same shape as X_i"
    assert grid_points.shape == (150, 3), "grid_points should be (150, 3)"
    
    # Check factor labels
    assert labels_i == ["Tanh", "ReLU", "Sigmoid"], "labels_i should contain factor levels"
    assert labels_j is None, "labels_j should be None for numeric dimension"
...
def test_generate_mesh_grid_with_factors_both_factors():
    ...
    def objective_two_factors(X):
        ...
        for params in X:
            ...
            score = l1 * 0.01 + activation_score.get(activation, 1.0) + optimizer_score.get(optimizer, 0.5)
            results.append(score)
        return np.array(results)
    ...
    assert X_i.shape == (2, 3), "X_i should be 2x3 (2 optimizer levels × 3 activation levels)"
    assert X_j.shape == X_i.shape, "X_j should have same shape as X_i"
    assert grid_points.shape == (6, 3), "grid_points should be (6, 3)"
    
    # Check factor labels
    assert labels_i == ["Tanh", "ReLU", "Sigmoid"], "labels_i should contain activation levels"
...
def test_generate_mesh_grid_with_factors_mean_computation():
    ...
    assert len(np.unique(dim1_values)) == 1, "Non-plotted dimension should have constant value"
...
def test_plot_important_hyperparameter_contour_with_factors():
    ...
    def objective_4d(X):
        ...
        for params in X:
            ...
            activation_score = {"Tanh": 1.0, "ReLU": 0.8, "Sigmoid": 1.2}
            score = l1 * 0.01 + activation_score.get(activation, 1.0) + lr * 10 + alpha * 5
            results.append(score)
        return np.array(results)
