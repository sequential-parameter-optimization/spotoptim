...
class MockModel:
    ...
    def predict(self, X):
        if X.shape[1] != self.n_features_in_:
            raise ValueError(f"X has {X.shape[1]} features, but MockModel is expecting {self.n_features_in_} features as input.")
...
def test_mo_xy_surface_mismatch_features():
    ...
    with pytest.raises(ValueError, match="expects 2 features, but 3 bounds were provided"):
        mo_xy_surface(models, bounds)
...
def test_model_predict_error_handling():
    ...
    model.predict.side_effect = ValueError("X has 3 features, but is expecting 2 features")
    ...
    with pytest.raises(ValueError, match="len\(bounds\) matches the model's input dimensionality"):
