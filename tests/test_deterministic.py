...
class TestDeterministicBehavior:
    ...
    def test_deterministic_with_provided_initial_design(self):
        ...
        opt1 = SpotOptim(fun=sphere, bounds=bounds, max_iter=10, n_initial=n_initial, seed=seed)
        result1 = opt1.optimize(X0=X0.copy())
        
        opt2 = SpotOptim(fun=sphere, bounds=bounds, max_iter=10, n_initial=n_initial, seed=seed)
    ...
    def test_reproducibility_across_dimensions(self):
        ...
        # 2D problem
        opt_2d_a = SpotOptim(fun=sphere, bounds=[(-5, 5), (-5, 5)], n_initial=5, seed=seed)
        X_2d_a = opt_2d_a._generate_initial_design()
        
        opt_2d_b = SpotOptim(fun=sphere, bounds=[(-5, 5), (-5, 5)], n_initial=5, seed=seed)
        X_2d_b = opt_2d_b._generate_initial_design()
        
        np.testing.assert_array_equal(X_2d_a, X_2d_b)
        
        # 3D problem
        opt_3d_a = SpotOptim(fun=sphere, bounds=[(-5, 5), (-5, 5), (-5, 5)], n_initial=5, seed=seed)
        X_3d_a = opt_3d_a._generate_initial_design()
        
        opt_3d_b = SpotOptim(fun=sphere, bounds=[(-5, 5), (-5, 5), (-5, 5)], n_initial=5, seed=seed)
