...
def test_example_2_custom_de():
    ...
    de_kwargs = {"maxiter": 200, "popsize": 30, "mutation": (0.6, 1.1)}
...
def test_example_3_lbfgs():
    ...
    lbfgs_kwargs = {"method": "L-BFGS-B", "options": {"maxiter": 100, "ftol": 1e-9}}
...
def test_example_4_nelder_mead():
    ...
    nm_kwargs = {"method": "Nelder-Mead", "options": {"maxiter": 500, "adaptive": True}}
...
def test_robot_arm_scenario():
    ...
    kernel = ConstantKernel(1.0, (1e-2, 1e12)) * Matern(
        length_scale=np.ones(n_dim), length_scale_bounds=(1e-4, 1e12), nu=2.5
    )
    surrogate = GaussianProcessRegressor(
        kernel=kernel, n_restarts_optimizer=10
    )  # Reduced restarts for test speed
