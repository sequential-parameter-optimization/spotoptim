...
class TestTerminationCriteria:
    ...
    def test_finite_iter_infinite_time(self):
        ...
        assert res.success is True or res.message.startswith(
            "Optimization terminated: maximum evaluations"
        )
    ...
    def test_verbose_output_sequential(self, capsys):
        ...
        opt = SpotOptim(
            fun=sphere_1d, bounds=[(-5, 5)], max_iter=6, n_initial=4, verbose=True
        )
        opt.optimize()
        ...
        assert (
            "Iter" in captured.out
            or "Best:" in captured.out
            or "evaluations" in captured.out
        )
    ...
    def test_verbose_output_parallel(self, capsys):
        ...
        assert any(
            ...
            for x in ["Iter", "Best", "evaluations", "Parallel", "Done"]
        )
