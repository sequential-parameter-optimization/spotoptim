...
def test_mo_mm_desirability_function():
    ...
    for i in range(y.shape[1]):
        model = RandomForestRegressor(n_estimators=10, random_state=42) # Reduced estimators for speed
    ...
    neg_D_mm, objectives_mm = mo_mm_desirability_function(
        x_test, models, X_base, J_base, d_base, phi_base, D_overall_mm, mm_objective=True
    )
