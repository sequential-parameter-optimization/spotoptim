import numpy as np
from spotoptim.sampling.designs import Designs


class Poor(Designs):
    """
    Class for generating poorly projected (collinear) designs.
    ...
    def generate_collinear_design(
        self, n_points: int, sigma: float = 0.01
    ) -> np.ndarray:
        """Generates a collinear design (poorly projected).

        Args:
            n_points (int): The number of points to generate.
            sigma (float): The standard deviation of the noise. Defaults to 0.1.
        ...
        y_coords = y_coords + np.random.normal(0, sigma, n_points)
